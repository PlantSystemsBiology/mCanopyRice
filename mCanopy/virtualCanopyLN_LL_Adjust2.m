...
function virtualCanopyLN_LL_Adjust2(canopyModelParameter_file, LN_adjustRatio,outputCanopyModelParameter_file)
...
for plantId = 1:PN
    ...
    [row1,col1]=size(modelA(modelA(:,PlantID_ind)==plantId, :));
    LNofPlant_A = row1;

    LN_adjustValue = round(LNofPlant_A*LN_adjustRatio) - LNofPlant_A; % 根据变化比例，计算改变值
    LN_adjustValue
    ...
    for i = 1:length(indexVector)
        ...
        % change leaf length to keep the total leaf area of the plant constant.
        % first, calculate the original canopy leaf area
        leafLengthV = modelA(modelA(:,PlantID_ind)==plantId, leafLength_ind);
        leafWidthV = modelA(modelA(:,PlantID_ind)==plantId, leafWidth_ind);
        ...
        leafLengthV = modelB(modelB(:,PlantID_ind)==plantId, leafLength_ind);
        leafWidthV = modelB(modelB(:,PlantID_ind)==plantId, leafWidth_ind);
        ...
        while abs(leafAreaNew - leafAreaOrigin)>1
           ...
           % meanV = mean(leafLengthV)
            leafLengthV(leafLengthV>72) = 70; % set those leaves longer than 72 to be 70.
            leafAreaNew = sum(leafLengthV .* leafWidthV * 0.7);
            newMinusOrign = leafAreaNew - leafAreaOrigin;
           % newMinusOrign
        end
      %  ratio
        modelB(modelB(:,PlantID_ind)==plantId, leafLength_ind) = leafLengthV;
