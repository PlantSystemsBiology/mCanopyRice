...
global TILLER_ANGLE;
global OS_DATA_MATRIX_COLUMN_NUM_POINTCLOUD;%  QF point cloud 2024-9-28

plantID = paramMatrix(1,1); % the first column is plantID and the same. 

M_plant = zeros(0,OS_DATA_MATRIX_COLUMN_NUM_POINTCLOUD); % empty single plant data matrix£¬one plant data¡£%  QF point cloud 2024-9-28
...
%    tillerID
    ...
    X = M_tiller(:,6); %  QF point cloud 2024-9-28
    X = X + X_move_outter;  
    Y = M_tiller(:,7); %  QF point cloud 2024-9-28
    Z = M_tiller(:,8); %  QF point cloud 2024-9-28
    ...
    [row3,col3] = size(X);
    M = [X, Y, Z];
    M_tiller(:,6:8)  = M;%  QF point cloud 2024-9-28
...
% CHECK face is face, not line%  QF point cloud 2024-9-28
% 
% smallFace_index = ...
