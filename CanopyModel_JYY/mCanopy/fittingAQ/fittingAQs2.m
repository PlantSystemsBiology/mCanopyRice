...
function AQfittingOutput_all = fittingAQs2(ID)
...
for i = 1:3 % stage, 1, 2, 3
    ...
    if i==2 && ID==1
        cali = [0.9124, 1.1364, 1]; % calibrate the curve according to the A1800 measured in field. 
    elseif i==3 && ID==1
        cali = [0.871863, 1.044646, 1]; % HS
    else
        cali = [1,1,1];
    end
    ...
    for j = 1:3 % genotype, 1, 2, 3
        ...
        d(1:row-1, 2:col) = d(1:row-1, 2:col).*cali(j);% calibrate factor for JYY at 0810 stage 
        
        AQfittingOutput = zeros(1, 10);
   ...
   %     for n=2:col  % for every single AQ curve
            ...
            AQfittingOutput(1,3:end) = [fitresult.Pmax, fitresult.phi, fitresult.theta, Rd,  gof.sse, gof.rsquare, gof.adjrsquare, gof.rmse];
